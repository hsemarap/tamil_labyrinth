...
if(in_array($_SESSION['userId'],$adminids))
{ 
...
<html>
<head>
<title>வலைபாயுதே | தமிழ் மன்றம்</title>
...
<input type='text' name='levelname' placeholder='Level URL'><br />
...
<label for='levelname'>URL:</label><br />
<input type='text' name='levelname' placeholder='Level URL' value='$levelname'><br />
<label for='newques'>Text:</label><br />
<textarea id='newques' style='height:100px;width:300px' name='question'>$text</textarea><br />
";
$fileselect1=$fileselect2=$fileselect3=$fileselect4="<option value=''> </option>";
...
foreach($files as $f)
	{
		...
		$fileselect4.="<option ".($src[3]==$f?" selected ":"")."value='$f'>$f</option>";
	}
$html.="
<select name='image1'>$fileselect1</select><img height='40' src='upload/$src[0]' />
<select name='image2'>$fileselect2</select><img height='40' src='upload/$src[1]' />
<select name='image3'>$fileselect3</select><img height='40' src='upload/$src[2]' />
<select name='image4'>$fileselect4</select><img height='40' src='upload/$src[3]' />
<br /><br />
...
if(isset($_POST['newquestion']))
{
	...
	$answer=$_POST['answer'];
	$query="INSERT INTO `tamil_questions` VALUES('','$level','$levelname','$question','$images','$answer')";
	//echo $query;
...
//	$answer=mb_convert_encoding($answer, "utf-8");
	$query="UPDATE `tamil_questions` SET `question` = '$question' , `images` = '$images' , `answer` = '$answer' ,`levelname` = '$levelname' WHERE `level` = '$level'";
	$query="UPDATE `tamil_questions` SET `question` = '$question' , `images` = '$images' , `answer` = '$answer' ,`levelname` = '$levelname' WHERE `tamil_questions`.`level` = $level";
